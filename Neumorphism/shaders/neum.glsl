...
void main() {
    ...
    vec4 glow = mix(mix(dark, light, clamp(_cblend, 0.0, 1.0)), color, ss(vec2(0.0, 4.0/_min), rdist - _innerpad));
    gl_FragColor = glow * ss(vec2(0.0, spread), rdist + _innerpad/2.0) * qt_Opacity;
}
